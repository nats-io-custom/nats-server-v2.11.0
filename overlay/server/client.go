...
type ClientOpts struct {
	...
	IgnoreSignature bool   `json:"ignore_sig,omitempty"`
...
func (c *client) processConnect(arg []byte) error {
	...
	// check to see if a JWT is NOT present in the client and set it if the server has a default one.
	if c.opts.JWT == _EMPTY_ && c.serverOpts.AuthSentinelJWT != _EMPTY_ {
		// this is to support a feature for dynamic accounts where the client is required to send sentinel Creds
		// for self host senarios this lets us set that JWT server side.
		// All the sentinel does here is lock in the context for the wellknown account that is handling auth.
		c.opts.JWT = c.serverOpts.AuthSentinelJWT
